...
final class EnvironmentHooks extends HookFeature {
    ...
    /**
     * Hook SubscriptionInfo methods for comprehensive SIM info spoofing.
     * From TikTokRegionHook 1.4.2.
     */
    private void hookSubscriptionInfo(RegionPreset preset) {
        String country = preset.code.toLowerCase(Locale.ROOT);
        String mcc = preset.operator.length() >= 3 ? preset.operator.substring(0, 3) : preset.operator;
        String mnc = preset.operator.length() > 3 ? preset.operator.substring(3) : "";
        int mccInt = 0;
        int mncInt = 0;
        try { mccInt = Integer.parseInt(mcc); } catch (NumberFormatException ignored) {}
        try { mncInt = Integer.parseInt(mnc); } catch (NumberFormatException ignored) {}

        try {
            Class<?> subInfoClass = Class.forName("android.telephony.SubscriptionInfo");
            for (Method method : subInfoClass.getDeclaredMethods()) {
                ...
                Object replacement = null;
                String id = null;
                if ("getCountryIso".equals(name) && method.getReturnType() == String.class) {
                    replacement = country;
                    id = "toki-subinfo-country";
                } else if ("getMcc".equals(name) && method.getReturnType() == int.class) {
                    replacement = mccInt;
                    id = "toki-subinfo-mcc";
                } else if ("getMnc".equals(name) && method.getReturnType() == int.class) {
                    replacement = mncInt;
                    id = "toki-subinfo-mnc";
                } else if ("getMccString".equals(name) && method.getReturnType() == String.class) {
                    replacement = mcc;
                    id = "toki-subinfo-mcc-string";
                } else if ("getMncString".equals(name) && method.getReturnType() == String.class) {
                    replacement = mnc;
                    id = "toki-subinfo-mnc-string";
                } else if ("getCarrierName".equals(name) && method.getReturnType() == CharSequence.class) {
                    replacement = preset.operatorName;
                    id = "toki-subinfo-carrier";
                }
                if (replacement != null) {
                    try {
                        Object val = replacement;
                        hook(method).setId(id).intercept(chain -> val);
                    } catch (Throwable ignored) {}
                }
            }
        } catch (ClassNotFoundException ignored) {
            logInfo("SubscriptionInfo class not found");
        } catch (Throwable error) {
            logError("Unable to hook SubscriptionInfo", error);
        }
    }

    /**
     * Hook ServiceState methods for carrier spoofing.
     * From TikTokRegionHook 1.4.2.
     */
    private void hookServiceState(RegionPreset preset) {
        try {
            Class<?> ssClass = Class.forName("android.telephony.ServiceState");
            for (Method method : ssClass.getDeclaredMethods()) {
                String name = method.getName();
                if (method.getParameterCount() != 0 || method.getReturnType() == void.class) {
                    continue;
                }
                Object replacement = null;
                String id = null;
                if ("getOperatorNumeric".equals(name) && method.getReturnType() == String.class) {
                    replacement = preset.operator;
                    id = "toki-servicestate-numeric";
                } else if ("getOperatorAlphaLong".equals(name)
                        && method.getReturnType() == String.class) {
                    replacement = preset.operatorName;
                    id = "toki-servicestate-alpha-long";
                } else if ("getOperatorAlphaShort".equals(name)
                        && method.getReturnType() == String.class) {
                    replacement = preset.operatorName;
                    id = "toki-servicestate-alpha-short";
                }
                if (replacement != null) {
                    try {
                        Object val = replacement;
            ...
            logInfo("ServiceState class not found");
        } catch (Throwable error) {
            logError("Unable to hook ServiceState", error);
        }
    }
}
