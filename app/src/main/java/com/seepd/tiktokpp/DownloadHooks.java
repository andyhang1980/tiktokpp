...
final class DownloadHooks extends HookFeature {
    ...
    /** TikTok saves media to DCIM/Camera via MediaStore. */
    void installLocation(ClassLoader classLoader, ModuleConfig config) {
        // Try multiple obfuscated class names for the media insert bridge
        int bridgeHooks = 0;
        String[][] bridgeCandidates = {
                {"X.0yn6", "X.0wj1"},  // 46.4.3 / 46.7.16
                {"X.0V1x", "X.0V1y"},  // 46.9.3 possible
        };
        for (String[] candidate : bridgeCandidates) {
            bridgeHooks = installMediaInsertBridge(
                    classLoader, candidate[0], candidate[1], config);
            if (bridgeHooks > 0) break;
        }
        ...
        logInfo("Download location hooks installed (" + bridgeHooks
